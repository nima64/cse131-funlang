...
fn compile_defn(defn: &Defn, mut ctx: CompileCtx) -> Vec<Instr> {
    ...
    // Use R11 as scratch register for address (caller-saved, safe to use here)
    // But Instr::Mov only supports Reg enum.
    // Reg enum has Rax, Rbx, Rcx, Rdx, Rsi, Rdi, Rsp, Rbp.
    // We can use Rcx or Rdx (arguments are passed in Rdi, Rsi, Rdx, Rcx, R8, R9).
    // Wait, we are passing arguments to `compile_me` later.
    // But here we are checking cache.
    // If cache hit, we jump.
    // If cache miss, we call `compile_me`.
    // We can use Rax for the value.
    // We can use Rcx for the address.
